...
class RaceDetector {
     ...
     * order according to trace
     *
     * @throws IOException
     */
    private void genTraceMessageHandlingConstraints() throws IOException {
        System.out.println( "[MinhaChecker] Generate receive-handler linkage constraints according to the trace" );
        solver.writeComment( "RECEIVE-HANDLER LINKAGE CONSTRAINTS" );
        String TAG = "RCV_HND";
        for(Map.Entry<SocketEvent, List<Event>> handlerEvents : traceProcessor.handlerEvents.entrySet()) {
            ...
            if (nextEvent != null) {
                ...
                rcvHndLinkage = rcvHndLinkage + " " + solver.cLt(handlerDelimiters.fst, nextEvent.toString());
            }
    ...
    /**
     * Handler mutual exclusion constraints state that handlers cannot execute in parallel, thus an handler
     * must execute either before or after any other handler.
     * Let H be set of all handlers in the same thread and for the same socket channel, and h_i and h_j be two different handlers \in H.
     * Then, the mutual exclusion constraints are written as:
    ...
    private void getHandlerMutexConstraints(Set<Pair<String, String>> handlerSet) throws IOException {
        ...
        Generator<Pair<String, String>> handlerCombinations = createSimpleCombinationGenerator(handlerVector, 2);
