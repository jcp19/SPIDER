...
public class MinhaCheckerParallel {
    ...
    while (pairIterator_i.hasNext()) {
      ...
      while (pairIterator_j.hasNext()) {
        ...
        if (rcv1.conflictsWith(rcv2)) {
          ...
          else raceCandidate = new CausalPair<SocketEvent, SocketEvent>(snd1, rcv2);
   ...
   * @return
   */
  public static int genSegmentOrderConstraints(List<Event> events, int segmentStart)
      throws IOException {
        ...
        if (rcv_i != rcv_j
          ...
          String handlerEnd_i =
              trace
                  .handlerEvents
                  .get(rcv_i)
                  .get(trace.handlerEvents.get(rcv_i).size() - 1)
                  .toString();
          ...
          String handlerEnd_j =
              trace
                  .handlerEvents
                  .get(rcv_j)
                  .get(trace.handlerEvents.get(rcv_j).size() - 1)
                  .toString();
  ...
  public static void genWaitNotifyConstraints() throws IOException {
    ...
    HashMap<SyncEvent, List<String>> binaryVars =
        new HashMap<
            SyncEvent,
            List<String>>(); // map: notify event -> list of all binary vars corresponding to that
    // notify
