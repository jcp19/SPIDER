...
import pt.minha.checker.solver.Z3Solver;
...
public class MinhaChecker {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            if (is != null) {
                ...
                checkConflictsMaximization();//checkConflicts();
                solver.close();//*/
    ...
    public static void loadEvents() throws IOException, JSONException {
        ...
        System.out.println("[MinhaChecker] load events from "+events);
    ...
    public static void genConflictCandidates() {
        System.out.println("[MinhaChecker] Generate data race candidate pairs");
    ...
    public static void checkConflicts() throws IOException{
        ...
        Iterator it = conflictCandidates.iterator();
        ...
        while(it.hasNext()) {
            MyPair<RWEvent,RWEvent> candidate = (MyPair<RWEvent,RWEvent>) it.next();
            boolean isConflict = solver.checkConflict(candidate.getFirst().toString(), candidate.getSecond().toString());
            if (!isConflict) {
                it.remove();
            }
        }
    ...
    public static void checkConflictsMaximization() throws IOException{

        solver.writeComment("CONFLICT CONSTRAINTS");
        Iterator it = conflictCandidates.iterator();
        Stats.totalCandidatePairs = conflictCandidates.size();
        long checkingStart = System.currentTimeMillis();
        String globalMaxConst = "";
        int counter = 0;
        for(MyPair<RWEvent,RWEvent> candidate : conflictCandidates){
            String first = candidate.getFirst().toString();
            String second = candidate.getSecond().toString();
            String dRaceVar = "DR_"+counter++;//first+"_"+second;
            solver.writeConst(solver.declareIntVar(dRaceVar,0,1));

            //Data Race constraint: (first == second && DR = 1) || (first != second && DR = 0)
            String andConstTrue = solver.cAnd(solver.cEq(first,second), solver.cEq(dRaceVar,"1"));
            String andConstFalse = solver.cAnd(solver.cNeq(first,second), solver.cEq(dRaceVar,"0"));
            String isDataRaceConst = solver.cOr(andConstTrue,andConstFalse);
            solver.writeConst(solver.postAssert(isDataRaceConst));

            globalMaxConst += (dRaceVar+" ");
        }
        System.out.println("[MinhaChecker] Generate global maximization constraint");
        solver.writeComment("GLOBAL MAXIMIZATION CONSTRAINT");
        solver.writeConst(solver.cMaximize(solver.cPlus(globalMaxConst,"")));
        solver.flush();
    ...
    public static void initSolver() throws IOException {
        ...
        System.out.println("[MinhaChecker] Init solver: "+solverPath);
    ...
    public static void genJoinExitConstraints() throws IOException {
        System.out.println("[MinhaChecker] Generate join-exit constraints");
